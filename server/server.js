...
app.post("/api/fetch-commits", async (req, res) => {
  try {
    const { repoUrl } = req.body;
    if (!repoUrl) {
      return res.status(400).json({ message: "Repository URL is required" });
    }
    exec(`python ../script/setup.py ${repoUrl}`, (error, stdout, stderr) => {
      if (error) {
      ...
      if (stderr) {
        ...
        return res
          .status(500)
          .json({ message: "Script failed", error: stderr });
      }

      console.log(`Python script output: ${stdout}`);
      return res
        .status(201)
        .json({ message: "Commits fetched successfully", output: stdout });
    });
  } catch (error) {
...
app.delete("/api/truncate-commits", async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Commits table truncated successfully" });
  } catch (error) {
